...
case class BinaryOr(a: BoolExpression, b: BoolExpression) extends BoolExpression {
   ...
   */
  override def mapSubexpressions(func: (IntExpression) => IntExpression): IntExpression = new BinaryOr(func(a).asInstanceOf[BoolExpression],
    func(b).asInstanceOf[BoolExpression])
}